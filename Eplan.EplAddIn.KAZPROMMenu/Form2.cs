...
namespace Eplan.EplAddIn.KAZPROMMenu
{
    ...
    public partial class Form2 : Form
    {
        ...
        private void загрузитьВсеКомплектыToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        void bw_DoWork1(object sender, DoWorkEventArgs e)
        {
            ...
            string buf = "";
            if ((selectNameKit == "") || (selectLocation == "")) return;
            using (LockingStep oLS = new LockingStep())
            { // ... доступ к данным P8 ...
                SelectionSet Set = new SelectionSet();
                Project CurrentProject = Set.GetCurrentProject(true);
                ...
                #region KITFound// Ищем Комплекты
                {
                    foreach (DeviceListEntry f in Devlist)
                    {
                        ...
                        if (buf == "KIT")
                        {
                            if (f.Properties.DEVICELISTENTRY_PARTNR.ToString().Remove(0, 4) != selectNameKit) continue;
                ...
                #region //Сравниваю с проектом
                {
                    foreach (ArticleReference ar in articref)
                    {
                        ...
                        //listBox1.Items.Add(ar.PartNr + "  " + ar.Properties.ARTICLEREF_COUNT);
                        progressBar1.Value++;
                        if (selectLocation != ar.Properties.DESIGNATION_FULLLOCATION) continue;
                        foreach (part cpart in filtpart)
                        {
        ...
        private void загрузитьОтдельноКомплектToolStripMenuItem_Click(object sender, EventArgs e)
        {
            using (LockingStep oLS = new LockingStep())
            { // ... доступ к данным P8 ...
                SelectionSet Set = new SelectionSet();
                ...
                #region //Поиск всех местоположений в ссылках изделий
                {
                    location.Clear();
                    foreach (ArticleReference ar in articref)
                    {
                        ...
                        foreach (part cpart in location)
                        {

                            if (ar.Properties.DESIGNATION_FULLLOCATION == cpart.design)
                            {
                                searchPLC = true;
                                break;
                            }
                ...
                #endregion
                SelectKIT sKIT = new SelectKIT(this);
                sKIT.FormClosing += SecondFormClosing;
                sKIT.Show();
        ...
        private void SecondFormClosing(object sender, FormClosingEventArgs e)// Получаем выделенные комплекты и местоположения
        {
            selectNameKit = (sender as SelectKIT).selectNameKit;
            selectLocation = (sender as SelectKIT).selectLocation;
            bw = new BackgroundWorker();
            if (bw.IsBusy) return;
            bw.WorkerSupportsCancellation = true;
        ...
        private void ExportExcelToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MainList.Count==0)
            {
                MessageBox.Show("Для экспорта данных пожалуйста импортируйте комплекты!");
                return;
            }
            menuStrip1.Enabled = false;
            ...
            try
            {
...

                {
                    ...
                    {
                        if ((MainList[i].NameKit == "Общий список изделий")) continue;
                        costprice1 = 0;
                        costprice2 = 0;
                        //Rng=xlSht.get_Range(xlSht.Cells[i + startline, 0], xlSht.Cells[i + startline, 7]);
                ...
                #region //КП
                {
                    ...
                    {
                        if ((SpecALL[i].NameKit == "Общий список изделий")) continue;
